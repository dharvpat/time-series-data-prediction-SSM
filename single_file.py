...
class SSM(OurModule):
    ...
    def fft_conv(self, u_input: torch.tensor, v_kernel: torch.tensor):
        ...
        u_f = np.fft.rfft(u_input, n=2*L) # (B H L)
        v_f = np.fft.rfft(v_kernel[:, :L], n=2*L) # (H L)

        y_f = oe.contract('b h l, h l -> b h l', u_f, v_f) 
        y   = np.fft.irfft(y_f, n=2*L)[..., :L]  # (B H L)
        return y
...
class ClosedLoopCompanionSSM(CompanionSSM):
    ...
    def fft_conv_d(self, u, v):
        L   = u.shape[-1]
        u_f = np.fft.rfft(u, n=2*L, dim=2) # (B H L)
        v_f = np.fft.rfft(v, n=2*L, dim=2) # (H D L)

        y_f = oe.contract('b h l, h d l -> b h l d', u_f, v_f) 
        y   = np.fft.irfft(y_f, n=2*L, dim=2)[:, :, :L, :] # (B H L D)
        return y
...
def triangular_toeplitz_multiply_(u, v, sum=None):
    ...
    u_f = np.fft.rfft(u_expand, n=2*n, dim=-1)
    v_f = np.fft.rfft(v_expand, n=2*n, dim=-1)
    uv_f = u_f * v_f
    if sum is not None:
        uv_f = uv_f.sum(dim=sum)
    output = np.fft.irfft(uv_f, n=2*n, dim=-1)[..., :n]
    return output
...
def triangular_toeplitz_multiply_padded_(u, v):
    ...
    n = u.shape[-1]
    assert n % 2 == 0
    u_f = np.fft.rfft(u, n=n, dim=-1)
    v_f = np.fft.rfft(v, n=n, dim=-1)
    uv_f = u_f * v_f
    output = np.fft.irfft(uv_f, n=n, dim=-1)
    output[..., n:] = 0
    return output
...
class TriangularToeplitzMultFast(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        g_f = np.fft.rfft(g_expand, n=2*n, dim=-1)
        gu_f = g_f * u_f
        gv_f = g_f * v_f
        d_u = np.fft.irfft(gv_f, n=2*n, dim=-1)[..., :n]
        d_v = np.fft.irfft(gu_f, n=2*n, dim=-1)[..., :n]
        d_u = d_u.flip(-1)
        d_v = d_v.flip(-1)
        return d_u, d_v
...
class TriangularToeplitzMultPaddedFast(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        g_f = np.fft.rfft(g_expand, n=n, dim=-1)
        gu_f = g_f * u_f
        gv_f = g_f * v_f
        d_u = np.fft.irfft(gv_f, n=n, dim=-1)
        d_v = np.fft.irfft(gu_f, n=n, dim=-1)
        d_u[..., n//2:].zero_()
        d_v[..., n//2:].zero_()
...
def _fft(x, N): return np.fft.rfft(F.pad(x, (0, 2*N-x.shape[-1])), n=2*N, dim=-1)
def _ifft(x, N): return np.fft.irfft(x, n=2*N, dim=-1)[..., :N]
...
def causal_convolution_inverse_wrong(u, v):
    ...
    v_f = np.fft.rfft(v_expand, n=2*n, dim=-1)
    uv_f = v_f / u_f
    x = np.fft.irfft(uv_f, n=2*n, dim=-1)[..., :n]
    return x
...
def krylov_toeplitz_(L, A, b, c=None):
    ...
    while not done:
        ...
        Af = np.fft.rfft(A, n=2*N, dim=-1)
        xf = np.fft.rfft(_x, n=2*N, dim=-1)
        xf_ = Af * xf
        x_ = np.fft.irfft(xf_, n=2*N, dim=-1)
        x_[..., N:] = 0
        x = torch.cat([x, x_], dim=0) # there might be a more efficient way of ordering axes
        if not done:
            A = np.fft.irfft(Af*Af, n=2*N, dim=-1)
            A[..., N:] = 0
    x = x[:L, ..., :N] # (L, ..., N)
    if c is not None:
        x = torch.einsum('l...n, ...n -> ...l', x, c)
...
class SpaceTime(nn.Module):
    ...
    def init_embedding(self, config):
        return init_embedding_fn(config)
